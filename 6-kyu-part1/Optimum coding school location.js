...
*/
var optimumLocation = function(s, l){
  let dist = []
  for(let i=0;i<l.length;i++){
    dist.push(s.map(v=>[Math.abs(v[0]-l[i].x)+Math.abs(v[1]-l[i].y)]))
  }
  dist=dist.map(v=>v.reduce((a,b)=>a+b*1,0))
  let min = dist.indexOf(Math.min(...dist))
  return `The best location is number ${l[min].id} with the coordinates x = ${l[min].x} and y = ${l[min].y}`
}
